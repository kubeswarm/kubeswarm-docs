...
Fan-out naturally increases the pending task count on the agent's queue. When [KEDA-based autoscaling](/finops/autoscaling) is enabled, this triggers pod scale-up:
...
Each subtask is a separate task execution that consumes tokens from the agent's [SwarmBudget](/finops/budget-management). The originating agent's own token usage for the fan-out/collect cycle is minimal (tool call overhead only). The real cost is in the subtask executions, which are tracked individually.
